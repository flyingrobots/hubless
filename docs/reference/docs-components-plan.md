...
- Shared snippets (progress, dependencies, tables, status summaries) can be embedded in any doc—`docs/PRD.md` and `docs/TechSpec.md` already import them.
...
3. Downstream docs (PRD, TechSpec, etc.) embed the same snippets so product and engineering specs share a single data source.

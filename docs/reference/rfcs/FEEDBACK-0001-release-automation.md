...
The release CLI landed as a smaller first slice: callers pass an explicit `--version`, tags are annotated by default, and GPG signing is opt-in with `--sign`. Conventional-commit bump inference, `VERSION` file updates, push semantics, and GitHub Release publishing remain follow-up work.
